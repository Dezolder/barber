...
const MasterBody = ({ masters, handleDelete, handleClick, handleEdit }) => {
  return (
        ...
        <div className="card m-4" key={item._id}>
          <div className="card-header">
            ...
            <div className="text-end">
              ...
              <button
                className="btn-secondary text-end m-1"
            ...
            <div>
              ...
              {item.availabilityForToday.map((i) => (
                <button
                  ...
                  onClick={() => handleClick(i, item._id, 'Today')}
                >
                  {i}
                </button>
              ))}
            ...
            <div>
              ...
              {item.availabilityForToday.map((i) => (
                <button
                  key={i}
                  className="badge bg-primary m-1"
                  onClick={() => handleClick(i, item._id, 'Tomarrow')}
                >
                  {i}
                </button>
              ))}
            ...
            <div>
              ...
              {item.availabilityForToday.map((i) => (
                <button
                  key={i}
                  className="badge bg-primary m-1"
                  onClick={() => handleClick(i, item._id, 'AfterTomarrow')}
                >
                  {i}
                </button>
              ))}
