...
import EditMaster from '../api/pages/editMasterPage'
...
const Masters = () => {
  ...
  useEffect(() => {
    // const { data } =
    axios
      .get('http://localhost:8080/api/select-master')
  ...
  const handleSort = (name) => {
    if (name === 'Name') {
      setAsSort({
        ...
        name2: 'class',
        name3: 'rate',
        sort1: asSort.sort1 === 'asc' ? 'desc' : 'asc',
        sort2: asSort.sort2 === 'asc' ? 'desc' : 'asc',
        sort3: asSort.sort3 === 'asc' ? 'desc' : 'asc'
      })
    }
    if (name === 'Rate') {
      setAsSort({
  ...
  const handleEdit = (id) => {
    history.push(`/select-master/edit/${id}`)
  }
  ...
  const handleDelete = (id) => {
    ...
    axios.delete(`http://localhost:8080/api/select-master/${id}`)
  }
  ...
  const filteredMasters = selectedClass
    ? masters.filter((master) => master.class === selectedClass)
    : masters
  const sortedMasters = _.orderBy(
    filteredMasters,
    [asSort.name1, asSort.name2, asSort.name3],
    [asSort.sort1, asSort.sort1, asSort.sort2, asSort.sort3]
  )
